 ...
 */
export function error(message, hint, code) {
 ...
 */
export function substep(message, status = 'pending') {
    const statusIcon = {
        pending: colors.dim('○'),
 ...
 */
export function updateSubstep(index, message, spinFrame) {
 ...
 */
export function progress(current, total, width = 30) {
 ...
 */
export function header(title, icon = icons.sparkle) {
    console.log();
 ...
 */
export function kv(key, value, color) {
 ...
 */
export function done(message, stats) {
    console.log();
...
export function createSpinner(message) {
    ...
    const start = () => {
        interval = setInterval(() => {
            process.stdout.write(`\r${colors.highlight(icons.spinner[frame])} ${currentMessage}   `);
    ...
    const stop = (success = true, finalMsg) => {
        if (interval) {
 ...
 */
export function validationError(field, expected, received) {
 ...
 */
export function jsonParseError(raw, position) {
...
//# sourceMappingURL=cli-ux.js.map