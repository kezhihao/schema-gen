...
type Format = 'json-schema' | 'typescript' | 'zod' | 'api-doc';
const ALL_FORMATS: Format[] = ['json-schema', 'typescript', 'zod', 'api-doc'];
...
program
  ...
  .action((jsonStrings, options) => {
    try {
      ...
      let formats: Format[] = ALL_FORMATS;
      if (options.formats !== 'all') {
        formats = options.formats.split(',').map((f: string) => f.trim() as Format);
        // Validate formats
      ...
      // Generate outputs
      const outputs: Record<Format, string> = {
